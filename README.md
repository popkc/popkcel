# popkcel
一个用纯C实现的跨平台的简单的异步网络库，带有纤程功能。

popkcel是“popkc的event loop”的意思，它支持epoll、kqueue和IOCP。相比于libuv，它的特点是轻量、简单，而且支持一种类似于纤程的功能，使得你可以像写同步代码一样地异步代码。
...
```
要使用popkcel的伪同步socket（语法类似同步操作，但事实上是使用类似纤程方法实现的异步操作），需要loop在运行状态下才行，而loop在运行状态时线程就被卡住了无法执行后续代码，所以我们需要用一个一次性回调函数，来让loop在运行时执行我们的函数。
```
...
```
以上是操作单个socket的例子，也是最常用的情况，不过我们有时候也需要在同一纤程中同时操作多个socket，比如你需要同时发送8个连接请求，那么等待他们一个返回后再执行下一个显然很傻，popkcel支持同时等待多个socket的伪同步操作，只要使用popkcel_multi*函数即可。
```
...
//纤程将在此处挂起，当所有连接出结果（成功或失败），或者15秒过后，纤程将继续执行后续代码
...
//使纤程重新在popkcel_multiOperationWait处挂起。若所有操作都有结果，或者超时了，那么popkcel_multiOperationReblock会立即返回，并执行后面的代码。
```
...
```
popkcel_initListener函数的第三个参数为1时表示监听ipv6地址，为2时表示同时监听ipv6和ipv4地址。windows下不支持同时监听ipv6和ipv4地址，你必须用两个Listener才行。
...
popkcel主要还是为linux写的，目前在windows下只支持单线程，也不支持在同一个Listener中，同时监听ipv6和ipv4地址。
